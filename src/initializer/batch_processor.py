...
class BatchProcessor:
        ...
        with self.container.get('sqlite_connector') as connector:
            for i in range(batch_count):
                ...
                workflow_batch_data = {
                    ...
                    'result_batch_dir': Path(self.config['workflow_output_directory']) / workflow_detail.calc_unique_name / 'results' / f'batch_{i+1}_{start_idx}_{end_idx}',
                    'script_path': Path(self.config['workflow_output_directory']) / workflow_detail.calc_unique_name / 'scripts' / f'batch_{i+1}_{calculation}_{start_idx}_{end_idx}.sh'
                }
    ...
    def process_icohp(self, workflow_detail: WorkflowDetail, batches: List[WorkflowBatchDetail], calculation: str) -> Tuple[List[WorkflowBatchDetail], List[WorkflowBatchExecution]]:
